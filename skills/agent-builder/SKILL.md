...
| "One dispatch per stage" was written Monday evening, after three fanned test-authors burned 462M tokens; by Wednesday two test-authors fanned on a five-srcRoot feature had burned **532M** — the rule's own escape hatch ("split by srcRoot") read as license, since neither dispatch was one-per-stage *or* one-per-srcRoot | An exception inside a rule needs its own bright line, or it is the hole the pressure finds. The split now means: one writer per srcRoot, named in the prompt, never two live in one |
| Two fixes for that same Monday incident shipped 60 seconds apart: removing `Bash` from `forge-test-author`'s tools held 23/23 runs; the prose rule aimed at the main loop was violated within 48 hours at larger scale | When a fix can be structural — a tool removed, a hook, a permission — prose addressed to the main loop under cascade pressure is not a substitute. Prefer the mechanism that has been seen to fire |
...
| Five PRDs were built over four weeks and **none was ever deleted** — `git log --diff-filter=D` over all history returned nothing. Two invisible causes: `git rm` **fails outright on an untracked PRD** and commits are batched, so PRDs were untracked exactly when close-out arrived; and others never reached close-out because the session ended inside the code-reviewer fix loop at 23:30 on a 1044M-token day | A terminal step must be written against the repo state it actually meets, not the tidy one — check the command runs before making it doctrine. And: **the last step of a pipeline needs a trigger that fires at session start, not at work's end.** "Done" becomes true at the end of the most expensive day, which is when it is least likely to be noticed |
| `forge-prd-author` was told "never cite another PRD — a citation to one rots", but the agents writing code and tests never got that rule. 160 source files, 20 of them test *names*, ended up citing PRD paths, so deleting a PRD would dangle every one | When a rule exists because an artifact is **temporary**, it binds every agent that can write a reference to it, not just the one that writes the artifact |
